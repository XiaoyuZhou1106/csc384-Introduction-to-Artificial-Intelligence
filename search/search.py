...
def depthFirstSearch(problem):
    ...
    while not open.isEmpty():
        ...
        if problem.isGoalState(final_state):
            return p[1]
        for succ in problem.getSuccessors(final_state):
            if not succ[0] in p[0]:
                new_path = [path for path in p[0]]
                new_path.append(succ[0])
                new_dir = [dir for dir in p[1]]
                new_dir.append(succ[1])
                new_cost = p[2] + succ[2]
                open.push((new_path, new_dir, new_cost))
    return []
        ...
        if p[2] <= seen[final_state]:
            ...
            for succ in problem.getSuccessors(final_state):
                ...
                if not succ[0] in seen or new_cost < seen[succ[0]]:
                    ...
                    new_dir = [dir for dir in p[1]]
                    new_dir.append(succ[1])
                    open.push((new_path, new_dir, new_cost), new_cost)
                    seen[succ[0]] = new_cost
    return []
    ...
    while not open.isEmpty():
        ...
        if p[2] <= seen[final_state]:
            if problem.isGoalState(final_state):
                return p[1]
            for succ in problem.getSuccessors(final_state):
                new_cost = p[2] + succ[2] - heuristic(final_state, problem) + heuristic(succ[0], problem)
                if not succ[0] in seen or new_cost < seen[succ[0]]:
                    new_path = [path for path in p[0]]
                    new_path.append(succ[0])
                    new_dir = [dir for dir in p[1]]
