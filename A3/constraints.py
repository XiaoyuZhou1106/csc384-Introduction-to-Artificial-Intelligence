        ...
        for m in qi.domain():
            for n in qj.domain():
                if m != n and abs(m - n) != abs(i - j):
                    satisfyingAssignments.append([m, n])
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        if var not in self.scope():
            return True
        #To test whether the values satisfied the required values in the given bound or not.
        def valsNotExceed(l):
            vals = [val for (var, val) in l]
            assigned_value = 0
            for value in vals:
                if value in self._required:
                    assigned_value += 1
            if self._lb <= assigned_value and assigned_value <= self._ub:
                return True
            else:
                return False
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsNotExceed)
        return x
