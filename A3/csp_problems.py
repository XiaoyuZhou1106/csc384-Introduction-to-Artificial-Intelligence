...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("name", row)])
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.extend([AllDiffConstraint("({})".format(colj), scope)])
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.extend([AllDiffConstraint("({},{})".format(i, j), scope)])
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    #First, construct variables: let every plane and the sequence of flight be the
    #varivale key, and its domain is all available flights that the plane can fly.
    all_flight_variables = []
    for i in range(len(planes_problem.planes)):
        temp_plane = planes_problem.planes[i]
        single_flight = []
        for j in range(len(planes_problem.can_fly(temp_plane))):
            domain = ['None'] #the plane may not fly all its possible flight, so there can be none flight
            if j == 0: #the initial flight
                domain.extend(planes_problem.can_start(temp_plane))
            else:
                domain.extend(planes_problem.can_fly(temp_plane))
            single_flight.append(Variable("{}, {}".format(temp_plane, j), domain))
        all_flight_variables.append(single_flight)

    #Then, find the constraints.
    all_constraints = []
    #Constraints for connected flights.
    available_connection = [list(conn) for conn in planes_problem.can_follow]
    #since plane may not fly all available flight, it will have none value. Thus,
    #the connection between flight and none must be available.
    for flight in planes_problem.flights:
        available_connection.append([flight, 'None'])
    available_connection.append(['None', 'None']) #also, all none is included.

    for m in range(len(all_flight_variables)):
        for n in range(len(all_flight_variables[m]) - 1):
            temp_connection = [all_flight_variables[m][n], all_flight_variables[m][n+1]]
            all_constraints.append(TableConstraint("flight", temp_connection, available_connection))

    #Constraints for maintenance
    possible_maintain = [fli for fli in planes_problem.maintenance_flights]
    possible_maintain.append('None') #That posistion can be no flight.
    for lst in all_flight_variables:
        for a in range(len(lst) - planes_problem.min_maintenance_frequency + 1):
            #in the sequence flights of a plane, every min_maintenance_frequency
            # plane must has a flight goes to the maintenance.
            all_constraints.append(NValuesConstraint("maintain station near {}".format(lst[a]),
                                                     lst[a:a+planes_problem.min_maintenance_frequency],
                                                     possible_maintain, 1, planes_problem.min_maintenance_frequency))

    #Constrains for the flight must be scheduled and no more than once.
    all_variables = []
    for p in all_flight_variables:
        for q in p:
            all_variables.append(q)
    for f in planes_problem.flights:
        all_constraints.append(NValuesConstraint("Single {}".format(f),
                                                 all_variables, [f], 1, 1))

    csp = CSP("planeSchedule", all_variables, all_constraints) #set to to your CSP
    ...
    for sol in solutions:
        planes_sol = {}
        for b in planes_problem.planes:
            planes_sol[b] = []
        for var, val in sol:
            if val == 'None':
                continue
            pla, pos = var.name().split(', ')
            planes_sol[pla].append((pos, val))
        for key in planes_sol:
            planes_sol[key] = sorted(planes_sol[key])
        single_sol = []
        for key2 in planes_sol:
            single_plane = [key2]
            for item in planes_sol[key2]:
                single_plane.append(item[1])
            single_sol.append(single_plane)
        result.append(single_sol)
